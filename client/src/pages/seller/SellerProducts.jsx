...
const SellerProducts = () => {
  ...
  const handleBulkSubmit = async (e) => {
    ...
    try {
      const categories = [{
        ...
        productsList: bulkProducts.map(p => ({
          name: p.name,
  ...
  const handleJsonSubmit = async () => {
    ...
    try {
      const res = await api.post('/seller/products/bulk-json', { categories: jsonPreview });
  ...
  const handleVoiceConfirm = async (productList) => {
    try {
      ...
      const categories = Object.entries(categoryMap).map(([category, productsList]) => ({
        category,
        productsList,
      }));
      await api.post('/seller/products/bulk-json', { categories });
      ...
      console.error('Failed to create product from voice:', error.response?.data);
  ...
  const handleDeleteAll = async () => {
    if (!window.confirm('Are you sure you want to delete ALL your products? This cannot be undone.')) return;
    try {
      const { data } = await api.delete('/seller/products/all');
      alert(`Deleted ${data.productsDeleted} products`);
      fetchProducts();
    } catch (error) {
  ...
  return (
    <div className="admin-layout">
      ...
      <main className="admin-content">
          ...
          <table className="admin-table">
            ...
            <tbody>
              {products.length > 0 ? products.map(product => (
                ...
                <tr><td colSpan="6" className="no-data">No products found. Add your first product!</td></tr>
              )}
            </tbody>
          </table>
        </div>
        ...
        {/* Edit/Add single product modal */}
        {showModal && (
          <div className="admin-modal-overlay" onClick={() => { setShowModal(false); setEditingProduct(null); }}>
            <div className="admin-modal" onClick={e => e.stopPropagation()}>
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="modal-actions">
                  <button type="button" className="modal-cancel-btn" onClick={() => { setShowModal(false); setEditingProduct(null); }}>
                    Cancel
                  </button>
